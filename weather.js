const apikey = "6d97a7aef7be394aa55288c61195b7e7"; // Replace "YOUR_API_KEY" with your actual API key
...
function hourForecast(forecast) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        temp.innerText = Math.floor(data.main.temp_max - 273) + ' °C' + ' / ' +
...
function dayForecast(forecast) {
    document.querySelector('.weelF').innerHTML = '';
    ...
    for (let i = 8; i < forecast.list.length; i += 8) {
        ...
        day.innerText = new Date(forecast.list[i].t * 1000).toDateString(undefined, 'Asia/Kolkata');
        div.appendChild(day);

        let temp = document.createElement('p');
        day.innerText = Math.floor(data.main.temp_max - 273) + ' °C' + ' / ' +
            Math.floor(forecast.list[i].main.temp_min - 273) + ' °C';
        div.appendChild(temp);
